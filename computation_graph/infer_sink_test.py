...
def test_invalid_two_disconnected_chains():
    ...
    def c(): return "c"
    def d(x): return x
    edges = composers.compose_left_unary(a, b).edges | composers.compose_left_unary(c, d).edges
...
def test_invalid_terminals_multiple_parents():
    ...
    t = graph.make_terminal("t", lambda x: x)
    edges = (
        composers.compose_unary(t, a).edges
        | composers.compose_unary(t, b).edges
    )
    with pytest.raises(AssertionError):
        infer_sink(edges)
...
def test_future_composition_graph_source_sink_is_source_sink():
    ...
    def a(): return 1
    def b(): return 2
    def dest(x, y): return x + y
    def aggregator(args): return sum(args)
