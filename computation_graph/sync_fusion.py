...
Off by default; the decision is read at graph-compilation time. The host
application can register a feature-flag check with `set_enabled_provider`
(e.g. LaunchDarkly's "sync-fuse" flag in nlu-runtime), which takes precedence
over the COMPUTATION_GRAPH_SYNC_FUSION environment variable fallback (set to
anything but "0"/"false" to enable). With COMPUTATION_GRAPH_DEBUG also set,
every evaluation runs both fused and unfused and asserts the same nodes were
computed.
"""
...
_enabled_provider: Optional[Callable[[], bool]] = None


def set_enabled_provider(provider: Optional[Callable[[], bool]]) -> None:
    """Register the host's feature-flag check (e.g. a LaunchDarkly lookup).

    Consulted on every graph compilation and takes precedence over the
    environment variable. Pass None to unregister. A provider that raises is
    treated as "no answer": the decision falls back to the environment
    variable, so a broken flag service cannot fail graph compilation.
    """
...
def enabled() -> bool:
    if _enabled_provider is not None:
        try:
            return bool(_enabled_provider())
        except Exception:
            logging.exception(
                "sync fusion enabled-provider raised; falling back to the environment variable"
            )
...
class FusionContext:
    ...
    sync_and_downstream: FrozenSet[base_types.ComputationNode]
    sync_not_downstream: FrozenSet[base_types.ComputationNode]
...
def _make_chain_executor(
    ...
    statically_concrete: FrozenSet[base_types.ComputationNode],
